...
function OrdersPageContent() {
  ...
  useEffect(() => {
    ...
    if (user) fetchOrders()
  }, [user, loading]) // ✅ SAME as yours (not changed)
    ...
    <div className="page-container">
      {successOrderId && (
        <div className="mb-8 bg-green-50 border-2 border-green-200 rounded-3xl p-6 flex items-center gap-4 animate-slide-up">
          ...
          <div>
            ...
            <p className="text-green-600 font-cute">
              Thank you for your order. We'll start preparing it with love!
            </p>
          </div>
        </div>
      )}
      ...
      {orders.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 font-cute mb-8">
            ...
            <div key={order.id} className="card border-2 border-blush/50 overflow-hidden">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-cute font-bold text-plum">
                      {order.order_number}
                    </p>
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right hidden sm:block">
                    ...
                    <p className="text-xs text-gray-400 font-cute capitalize">
                  ...
                  {expanded === order.id
                    ? <ChevronUp size={18} className="text-plum" />
                    : <ChevronDown size={18} className="text-plum" />
                  }
                </div>
              </div>
              ...
              {expanded === order.id && (
                <div className="border-t border-blush px-5 pb-5 animate-slide-up">
                  ...
                  <div className="mt-4 space-y-3">
                    <h4 className="font-cute font-bold text-gray-600 text-sm">
                      Items ordered:
                    </h4>
                    ...
                    {order.items?.map((item, i) => (
                      <div key={i} className="flex items-center gap-3 bg-cream rounded-2xl p-3">
                        ...
                        <div className="flex-1">
                          <p className="font-cute font-semibold text-plum text-sm">
                            {item.name}
                          </p>
                  ...
                  {order.address_snapshot && (
                    <div className="mt-4 text-sm font-cute text-gray-600">
                      ...
                      <p>
                        {order.address_snapshot.address_line1}, {order.address_snapshot.city}, {order.address_snapshot.state} - {order.address_snapshot.pincode}
                      </p>
                    </div>
                  )}
                  ...
                  <div className="mt-3 flex items-center gap-2 text-sm font-cute">
                    ...
                    <span className="text-gray-400">
                      via {order.payment_method === 'cod' ? 'Cash on Delivery' : 'Online'}
                    </span>
                  </div>
...
/* ✅ WRAPPER FIX (IMPORTANT FOR VERCEL BUILD) */
export default function OrdersPage() {
  return (
    <Suspense fallback={<div className="page-container text-center py-20">Loading...</div>}>
      <OrdersPageContent />
    </Suspense>
  )
}