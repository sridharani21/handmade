...
import { Plus, Edit2, Trash2, X, Check, AlertCircle, Search, ImagePlus, RefreshCw } from 'lucide-react'
 ...
 const fetchCategories = useCallback(async () => {
  ...
  try {
    ...
    if (error) {
      ...
      console.log('Categories loaded:', data) // you can see this in browser console
      setCategories(data || [])
    }
  } catch (err) {
  ...
  // Refresh categories every time modal opens
  useEffect(() => {
    if (showModal) {
      fetchCategories()
    }
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl border border-blush/50 overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
                  ...
                  <tr key={product.id} className="hover:bg-cream/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-cute font-semibold text-plum text-sm max-w-[160px] truncate">
                            {product.name}
                          </p>
                          {product.is_featured && (
                            <span className="badge bg-butter text-plum text-xs">✨ Featured</span>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      {product.compare_price && (
                        <p className="text-xs text-gray-400 line-through font-cute">₹{product.compare_price}</p>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          ...
                          className="p-2 rounded-xl hover:bg-blush transition-colors text-plum"
                        >
                          <Edit2 size={15} />
                        </button>
                        <button
            ...
            {filtered.length === 0 && (
              <div className="text-center py-12 text-gray-400 font-cute">
                No products found 🌸
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-xl animate-slide-up">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-red-500 text-white py-3 rounded-full font-cute font-semibold hover:bg-red-600 transition-colors"
              >
                Delete
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <label className="block text-sm font-semibold text-gray-700 mb-1 font-cute">
                    Product Name *
                  </label>
                  <input
                    className="input-field"
                    placeholder="e.g. Handmade Macramé Earrings"
                    value={form.name}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="block text-sm font-semibold text-gray-700 font-cute">
                      Category
                    </label>
                    {/* Refresh button */}
                    <button
                      ...
                      className="flex items-center gap-1 text-xs text-plum hover:text-mauve transition-colors font-cute"
                  ...
                  {categories.length === 0 && (
                    <p className="text-xs text-orange-500 font-cute mt-1">
                      No categories yet — go to Settings to add one, then click Refresh above
                    </p>
                  )}
                </div>
                <div>
              ...
              {/* Images */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2 font-cute">
                  Product Images
                </label>
              ...
              <div>
                ...
                <input
                  className="input-field"
                  placeholder="Wipe clean, avoid water..."
              ...
              <div>
                ...
                <input
                  className="input-field"
                  placeholder="handmade, gift, earrings"
                  value={form.tags}
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <label
                    key={tog.key}
                    className="flex flex-col items-center gap-2 p-3 bg-cream rounded-2xl cursor-pointer hover:bg-blush/30 transition-colors"
                  >
                    <span className="text-2xl">{tog.emoji}</span>
                    <span className="text-xs font-cute font-semibold text-gray-600 text-center">
                      {tog.label}
                    </span>
                    <div
                      className={`w-10 h-5 rounded-full transition-colors relative ${form[tog.key] ? 'bg-plum' : 'bg-gray-200'}`}
